...
docker rm -f my-rabbitmq >nul 2>&1
...
docker rm -f my-nginx-lb >nul 2>&1
docker run -d --name my-nginx-lb -p 8080:8080 ^
  -v "%CONF%:/etc/nginx/nginx.conf:ro" ^
  -v "%LOGS%:/logs" ^
  nginx:alpine >nul
if errorlevel 1 (
    echo Failed to start my-nginx-lb
    pause
    exit /b 1
)
